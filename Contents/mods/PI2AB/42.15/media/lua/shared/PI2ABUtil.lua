...
function PI2ABUtil.GetScythingAction(queue)
    for i = 1, #queue do
        local action = queue[i]
        if action.Type == "ISScything" then
            return action,i
        end
    end
    return nil
...
function PI2ABUtil.GetSqUniqueId(prefix, sq)
    if not prefix or not sq then
        return nil
    end
    ...
    local pre = ''
    if prefix then
        pre = tostring(prefix) .. "_"
    end

    return pre .. x .. "_" .. y .. "_" .. z
end
